...
function send(file,data,funcion){
	log(domain+file);
	showProgress();
...
// 			log('request=>'+JSON.stringify(data)+"\n"+'response=>'+JSON.stringify(json));
// 		}
// 		 return json; // 🔑 Esto permite usar await
// 	})
// 	.catch(function(err){ hideProgress(), show('Error la Operación'), log('request=>'+JSON.stringify(data)+"\n"+'error response=>'+JSON.stringify(err)) });
// }
...
const ffff = item?.U_Origen === 'CREDITO'
  ? `
    <table>
      <tr>
        <td colspan="9" align="left">
          <strong>Razón Social:</strong> ${item.U_RazonSocial ?? ''}
        </td>
        <td colspan="3" align="left">
          <strong>N.º de Traspaso de Material:</strong> ${item.U_Observaciones ?? ''}
        </td>
      </tr>
    </table>
  `
        ...
        <tr>
		 ...
		 <p>BOLETA DE PESAJE (${origen}) - Nº Ticket: ${data[0]['U_NroTicket']}</p> <p>========================================</p> </div> <div> <p>RAZON SOCIAL : ${data[0]['U_RazonSocial']}</p>
		  ...
		  <body>
			  ...
			  <div class="center">
				  <p>BOLETA DE PESAJE d (${origen}) - Nº Ticket: ${data[0]['U_NroTicket']}</p>
				  <p>========================================</p>
			  </div>
			  <div>
...
function printInfo(ticket,dataFact,dataDoc){
	...
	dataFact.forEach(function(item,index,array){
		...
		if(item['U_Origen']=='CONTADO'){
			...
			if(item['U_Origen']=='CREDITO'){
              factura=`<div class="input-field col s4"><input value="${item['U_RazonSocial']}" type="text" maxlength="50"><label class="label active">RAZON SOCIALj</label></div>
			  <div class="input-field col s4"><input value="${item['U_Observaciones']}" type="text" maxlength="50"><label class="label active">N.º de Traspaso de Material</label></div>`;
			  creditor='hide';
			}else{
              factura=`<div class="input-field col s4"><input value="${item['U_RazonSocial']}" type="text" maxlength="50"><label class="label active">RAZON SOCIALj</label></div>
			  <div class="input-field col s4"><input value="${item['U_Observaciones']}" type="text" maxlength="50"><label class="label active">N.º de Traspaso de Material</label></div>`;
		...
		<div class="input-field col s4"><input value="${item['U_Origen']}" type="text"><label class="label active">TIPO DE SERVICIOcc</label></div>
		${factura}
		</div>
		<div class="col s12 card">
