...
//$data=query("select * from errores where date(fecha)='$date'");
	...
	value('$data', '$datoo', '$usuario', '$data_datos')");
        ...
        $data[0]["DocumentLines"][0]["DocumentLineAdditionalExpenses"] = array(
            array("LineNumber" => 0, "GroupCode" => 0, "ExpenseCode" => 4, "LineTotal" => $precio * $cant * 0.03, "LineTotalFC" => $cant * $precio * 0.03, "LineTotalSys" => $cant * $precio * 0.03),
...
case 'get_usuario_disponible':
    ...
    $almacen = addslashes($data[0]['ALMACEN']);
      ...
      $buscar_almacen = query_entrega_db("select *  FROM almacen_activo cc  WHERE cc.ALMA = '$almacen'  AND cc.Estado = 1 ");
        echo json_encode([
            "success" => false,
            "message" =>  $buscar_almacen,
            "data" => []
        ]);
